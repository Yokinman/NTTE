...
#define weapon_fire(w)
  ...
  sound_play_pitchvol(sndCrystalJuggernaut, 0.7 * _p, .7);
  sound_play_pitchvol(sndOasisExplosionSmall, 2 * _p, .7);
  sound_play_pitchvol(sndOasisExplosion, 4 * _p, .7);
	sound_play_pitch(sndOasisMelee, _p);
  sound_play_pitch(sndHammer, _p);
	...
	motion_add(w.ang, 2);
	 ...
	 // Slash:
	var _len = w.length + (12 * skill_get(mut_long_arms)),
