...
#define SpiderTangle_create(_x, _y)
	with(instance_create(_x, _y, CustomProjectile)){
		 // Visual:
		spr_appear = spr.SpiderTangleAppear;
		spr_disappear = spr.SpiderTangleDisappear;
		sprite_index = spr.SpiderTangle;
		image_speed = 0.4;
		depth = 4;
		
		 // Vars:
		mask_index = sprTangleSeedB;
		on_ground = false;
		despawn_timer = 300;
...
#define SpiderTangle_step
	if(on_ground){
		speed = 0;
		// image_angle = 0;
		
		despawn_timer -= current_time_scale;
		if(despawn_timer <= 0 && sprite_index == spr_appear){
			...
			mask_index = mskNone;
			 ...
			 // Sounds:
			sound_play_pitch(sndPickupDisappear, 1.1 + random(0.2));
		}
	}
	...
	if(projectile_canhit(other)){
		...
		if(!on_ground){
			...
			sprite_index = spr_appear;
			 ...
			 // Sounds:
			sound_play_hit(sndPlantSnare, 0.3);
		}
		
		 // Slow Enemies:
		else with(other){
			x = lerp(x, xprevious, 0.9);
			y = lerp(y, yprevious, 0.9);
		}
	}
	
#define SpiderTangle_wall
	if(!on_ground){
		on_ground = true;
		sprite_index = spr_appear;
