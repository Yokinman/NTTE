	...
	/// Water Wading Objects:
		...
		with(instances_matching_gt(instance_rectangle_bbox(_x1 - 24, _y1 - 24, _x2 + 24, _y2 + 24, global.sea_inst), "wading", 0)){
			...
			if(_isPlayer){
			 ...
			 // Call Draw Event:
			y += _z;
			if("right" in self || "rotation" in self || ("on_draw" in self && !is_undefined(on_draw))){
			...
			with(_surfSwimTopSub){
				...
				surface_set_target(_surfSwimTop.surf);
					...
					var	_x = x - _surfSwimTopX,
						...
						_charm = variable_instance_get(other, "ntte_charm");
					...
					if(!is_undefined(_charm) && _charm.charmed){
						 // Outlines:
						if(_charmOutline){
							...
							for(var a = 0; a < 360; a += 90){
								draw_surface_part_ext(surf, 0, 0, _w, _h, _x + dcos(a), _y - dsin(a), _surfSwimTopScale / scale, _surfSwimTopScale / scale, c_white, 1);
							}
							draw_set_fog(false, 0, 0, 0);
						}
						
						 // Self:
						draw_surface_part_ext(surf, 0, 0, _w, _h, _x, _y, _surfSwimTopScale / scale, _surfSwimTopScale / scale, c_white, 1);
					 ...
					 // Self:
					else draw_surface_part_ext(surf, 0, 0, _w, _h, _x, _y, _surfSwimTopScale / scale, _surfSwimTopScale / scale, c_white, 1);
					
					 // Water Interference Line:
					draw_set_fog(true, c_white, 0, 0);
					draw_surface_part_ext(surf, 0, _h, _w, max(1, scale), _x, _y + (_h * (_surfSwimTopScale / scale)), _surfSwimTopScale / scale, _surfSwimTopScale / scale, c_white, 0.8);
					draw_set_fog(false, 0, 0, 0);
